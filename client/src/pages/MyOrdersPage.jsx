...
function MyOrdersPage({ onBack, onSignupClick, onLoginClick, onAdminClick, onLogout, onViewOrderDetail }) {
  ...
  const fetchAllOrders = async () => {
    try {
      ...
      const response = await fetch('/api/orders/my?limit=1000', {
        method: 'GET',
        headers: {
  ...
  const fetchOrders = async (status = null) => {
    try {
      ...
      const result = await response.json();
      console.log('[MyOrdersPage] 주문 목록 응답:', result);
      if (result.success) {
        const ordersData = result.data || [];
        console.log('[MyOrdersPage] 주문 개수:', ordersData.length, '선택된 상태:', status);
